...
| R33 | **需求里没写的东西一律不加**，两类都算：① 玩家可见的说明性文字（按钮 / 开关下方的解释、面板页脚与页面说明、图鉴 / 帮助里的补充段）；② **显示项与交互**（等级、计数、状态标签、额外按钮、额外的面板）。没明说就不加，也不要「顺手补一句 / 顺手加一格」—— 缺文案就留空，想加先问。反例（2026-09-18）：技能槽上顺手写了 `Lv.N`；军事训练卡片另起一套 `.military-card` 而没有复用工坊的 `.shop-item` |
| R34 | **难度与特性的数值只有一个派生入口**：读敌人数值一律走 `currentEnemy()` / `getBattleEnemy()` / `battleEnemyOf()`（含 `prepareEnemy` 取的起始血量），特性判定只有 `hasTrait()`。详见 §7.20 |
| R35 | **手动模式**：玩家侧不自动出手、敌人照常出手；【普通攻击】的冷却跟随出手间隔（手动不会更快）；冷却进存档（`adventure.skillTimers`）随 tick 递减。详见 §7.20 |
| R36 | **升华不给提示**：`Item.growth` 的阶与升华不写进任何文案（描述 / 图鉴 / 帮助 / 公告），**界面上也不显示「第几阶」**（详情 / 图鉴 / 卡片都没有这个字段），只以右键菜单的一个按钮出现；升阶会**清空精炼**、不可撤销。详见 §7.20 |
| R37 | **新机制解锁必须配一次性引导**：新系统 / 新玩法（新页签、新模式、新操作方式）都要在 `unlockNotices` 里补解锁提示、并在 `guide.ts` 的 `GUIDES` 里补一段同 id 的引导（含「亲手做一次」与「后果 / 风险」两条）；只发一条提示不算做完。详见 §6.13 |
| R38 | **规则没写清的情形先问，不要自己拍板**：需求只覆盖了一种情形时，另一种情形**不要顺手定下来**（也不要拿「改动前的行为」当默认）—— 先按「什么都不做」做最小实现，同时在回复里把这个问题提出来，等玩家定调再补。反例（2026-09-18）：换难度时「Boss 已被击败」的情形被自作主张定成「即时出新 Boss」，做完才提 |
...
色板一次备足 **17 档**（`--rarity-0` ~ `--rarity-16`，见 `:root`），类名由 `rarityClass()` 从 `config/rarity.ts` 的 `className` 生成。**目前已用到 6 档**（⚠️ **两套装备可以同色** —— 稀有度只是色彩档、不是进度刻度，颜色重叠不代表强度重叠）：

| id | 类 | 名称 | 令牌 | 分配给 |
| --- | --- | --- | --- | --- |
| 0 | `.rarity-gray` | 灰色 | `--rarity-0` | 废弃边境 |
| 1 | `.rarity-white` | 白色 | `--rarity-1` | 余烬矿脉 / 核心深井 |
| 2 | `.rarity-blue` | 蓝色 | `--rarity-2` | 熔火裂谷 / Boss 独占（熔核之扉） |
| 3 | `.rarity-green` | 绿色 | `--rarity-3` | **空着** —— Boss 之后的新区域 / 新世界从这里往上取 |
...
⚠️ **降档是允许的**（2026-09-18：核心套从蓝 2 → 白 1、熔火套从绿 3 → 蓝 2，Boss 独占取蓝 2）：稀有度不参与任何数值，改它只影响物品名的颜色。改完记得同步 `config/items.ts`、`config/rarity.ts` 与本表三处。
...
| 类 | 类别 | 值 | 说明 |
| --- | --- | --- | --- |
| `.affix-offense` | 进攻 | `--red` | 直接提升输出：锋锐、余烬核心 |
...
新系统 / 新玩法（新页签、新模式、新操作方式）**只发一条解锁提示不算做完** —— 提示只说「有什么解锁了」，
玩家看完还是不知道怎么用。做法三步，一条都不能省：

| # | 做什么 | 落在哪 |
| --- | --- | --- |
| 1 | 补一条**解锁提示**（没有「条目」可挂的机制手写在 `unlockNotices` 末尾），它的 `id` 就是引导的键 | `game-state.ts` 的 `unlockNotices` |
| 2 | 补一段同 `id` 的**引导**（2~4 步） | `src/guide.ts` 的 `GUIDES` |
| 3 | 引导里必须有一步是**亲手做一次关键操作**（`requireClick`；右键 / 拖拽类的用 `click` + `waitFor`） | 同上 |

引导的四步套路：**指路（点导航过去）→ 认面板 → 亲手做一次 → 说清后果与风险**（冷却、代价、挂机会不会出事）。
「后果与风险」这一步别省：手动模式那句「挂机时记得切回自动」就是靠它讲出去的。

⚠️ 反过来，**刻意要让玩家自己发现的东西不写引导**（例如可成长饰品的升华，见 §7.20 / R36）——
不写引导 ≠ 漏了引导，两种情况在文档里都要说清是哪一种。
...
- **丢弃有两个入口**：右键菜单里的「丢弃」，以及 **Ctrl + 左键点卡片**（省掉右键那一步）。两者共用 `discardFromCard()`，
  守卫与后果必须一致（系统物品不给丢；手里不止一份时先弹「保留一份 / 丢弃全部」）。使用模式（`body.enhancing`）中 Ctrl + 左键不丢东西 —— 那时点卡片是取消。
...
⚠️ **例外：一次性强化项（余烬核心）的 `desc` 要写数值** —— 它只能刻一条、数值也不成长，那三行对它没有意义（详情里不显示），所以数字直接进描述；而且 `desc` 与 `summary` 都从 `EMBER_BURST` 生成，改数值只改那一处。

| 类别 | 现有词条 | 强化道具 | 清洗剂 |
| --- | --- | --- | --- |
| 进攻 | 锋锐（攻击力 %）、余烬核心（技能） | 锐化油、余烬核心（强化道具，限武器 · 一次性 · 每件武器一条） | 卸刃剂 |
...
| 坚韧 | 12 | 3 | 24 | 5 |
| 铁壁 | 4 | 1 | 8 | 5 |
| 余烬核心 | 100 | — | —（只能刻一条，强度固定） | 1 |
| 勤务 | 8 | 2 | 16 | 5 |

⚠️ **改 `step` 前先算一遍这个数** —— `step` 越小，同一个上限要的道具越多（词条本身不会变弱，只是更难顶到）。掉落率那边同步压过一轮：`config/zones.ts` 里强化道具的 `chance` 统一 ×0.6（见该文件头部的说明）。
⚠️ **2026-09-18 两次单独下调了余烬核心**（它成了一次性强化项，需求量本来就小）：普通怪 **0.2%~0.6%**、核心泰坦 0.6%、裂谷巨像 0.5%、熔核守卫（Boss）**4% 且最多 1 件**。
⚠️ 掉率低到千分之几之后，图鉴的角注**不能再用 `Math.round(chance * 100)`**（会显示成「0%」，等于告诉玩家不会掉）—— 用 `wiki.ts` 的 `chanceText()`：≥1% 取整、<1% 留一位小数。

**强化物可以在物品定义上挂两条限制**（`types.ts` 的 `Item` 字段）：

| 字段 | 含义 | 现状 |
| --- | --- | --- |
| `affixEquipType` | 只能刻在**这类装备**上（`equipType` 下标） | 余烬核心 = 只吃武器 |
| `affixUnique` | **不能重复**：目标已经有这条词条就直接拒绝 | 余烬核心 = true |
| 一件装备几条词条 | **最多 3 条**（`AFFIX_MAX_COUNT`）：满了就不接受新词条，被拒时道具**不消耗** | 老存档里已经超过 3 条的**不裁**（裁掉等于删玩家的东西），只是不再往上加 |

- 判定**只有一处**：`game-state.ts` 的 `grantAffixTo`（被拒时返回 `false` ⇒ 调用方不消耗道具）。**别在页面里重写这条判定**。
- 页面与图鉴只**读这份声明**来提示范围：使用模式的提示条（`pages/inventory.ts` 的 `syncUseHint`）会写成「点击一件武器」；图鉴的「附加词条」小节按限制改写 —— 余烬核心因此**没有**「重复使用再 +N」那一句，详情里也**不列**「首次 / 重复 / 最高」三行（见 `pages/inventory.ts` 的 `statLine`），`summary` 里同样不带 `{value}` / `{cap}`。
- 被拒的原因写进战斗日志（`addLog`）：点错目标时界面不会有别的反馈，那一行是唯一的解释。老存档里已经刻上去的**不会消失**，只是不能再往同一件上叠。
...
| 还没实现的 helper | **不挂在这一批上**：`boss` 归批次 3（跟 Boss 区域一起，先要有「每只怪击杀记录」字段，见 `庇护所扩展方案.md` §4.6）、`challenge` 归批次 4（§4.9）；`event` 无依赖（三种胜场计数已在存档里），要用时再加。加它们时照 `map` 的写法补，并且**必须给出 `text()`** |
...
| 2-3 穿过裂谷 | 熔火裂谷 | 区域击杀 **6000**（原为「2400 + 余烬核心 ×250」，见下面的 ⚠️） | 解锁区域「熔核之扉」（Boss 区域，见 §7.20） |
| 2-4 击碎熔核 | 熔核之扉 | 击败「常规」难度的熔核守卫 1 次 | 解锁【手动模式】与研究基地「军事训练」页签（读 `bossClears` 的第 0 位，见 §7.20） |
...
装甲板在核心深井 ≈1440/h —— 2-1 / 2-2 的约 3 / 3 小时就是这么来的。

⚠️ **2-3 不走这条路**（2026-09-18）：余烬核心的掉率下调到千分之几（熔火裂谷 ≈2.5 个/小时）之后，
原来那条「收集余烬核心 ×250」要上百小时（≈12 万杀），于是**整条撤掉** —— 这一节的长度改由**击杀数**承担
（6000 杀 ≈ 5 小时，仍是三节里最长的一节）。余烬核心此后只服务「给武器刻词条」这一件事。
...
### 7.20 Boss 区域 / 特性 / 手动模式 / 军事训练 / 升华（`config/zones.ts` + `game-state.ts` + 页面）

**一、Boss 区域与难度档**（`config/zones.ts`）

| 项 | 规则 |
| --- | --- |
| `Zone.kind` | `'combat' \| 'boss' \| 'camp'`，**可选**：`camp` 仍由「`enemyIds` 为空」推导（`isCampZone()` 没改）。新代码一律走 `zoneKind(zoneId)`，不要再写 `enemyIds.length` 判断 |
| `Zone.spawnCooldown` | **复活计时**：**击杀之后**到下一只出现的秒数（缺省 = 全局 `SPAWN_COOLDOWN` 3 秒）；Boss 区域给 60 秒。⚠️ **进入区域 / 换难度不走它** —— 那两条路用全局值（`getSpawnCooldown()`），混用会变成「每次进门都要干等一场复活」。界面的环形进度按 `adventure.spawnTotal`（这一轮的总时长）画；开发者面板的覆盖对两者都优先 |
| 换难度的刷新规则 | `selectDifficulty()`：场上那只**还站着（未被击败）**⇒ 把它作废，起一轮**全局**刷怪冷却才有下一只（换档不是免费的即时刷新）。**场上没有敌人（已击败、还在复活计时里）⇒ 只改档位：不重刷、也不动那个计时**，复活计时跑完出来的是新档那只（2026-09-18 拍板；边界情形先问，见 R38）。判据 `running && spawnTimer <= 0`（与 `useSkill` 一致） |
| `Zone.difficulties` | 一档一条 `{ name, hp, attack, defense, interval, gold?, dropTier }`。**下标即 `adventure.difficulty`** —— 只能追加。⚠️ **没有 requires / immune 了**（门槛特性整条移除，见下面「二、特性」） |
| 三档的关系 | **逐档全面更强**（生命 / 攻击 / 防御单调上升、出手越来越快）；不再是「三种体态各偏一头」（那版的生命是倒着排的，越高档越脆，读起来就是错的）。高防的档靠【破甲】与更多精炼 / 词条去破 |
| 校准基准 | 「常规」按 **全极致深井套 + 锐化油 / 生命之种 / 铁壁涂层打满** = 攻 114 / 防 56 / 血 582 / 回复 2 / 间隔 2.2 校准：一场打掉约 86% 生命（一场一条命）。⚠️ 敌人的**攻击必须显著高于这个基准的防御（56）**，否则减法一算就剩个位数 —— 换 Boss 数值时先跑一遍 庇护所扩展方案 §8 的复算口径 |
| 单一派生入口 | 读敌人数值**一律走 `currentEnemy()` / `getBattleEnemy()` / `battleEnemyOf(id)`**（敌人表原值 + 当前档位覆盖），`prepareEnemy` 的起始血量也从这里取。直接查 `enemyTable` 就会出现「界面一档、实打另一档」（R34） |
| 难度档的适用面 | `battleEnemyOf` 只在「这只怪属于当前区域」时套档位 —— 换区域后、还没刷出新敌人之前，`adventure.enemyId` 可能还指着上一区的怪 |
| 掉落档位 | `DropEntry.tier` 缺省 = 所有难度都掉；写了就只在 `dropTier` 相等的档掉（`grantDrops` 一行过滤）。`discoveredDrops` 存物品下标，不受影响 |
| 通关记录 | `enemyWins[enemyId]` 是**总数**（`unlockBy.boss()` 读它）；`bossClears[enemyId]` 是**难度位掩码**（第 n 位 = 第 n 档通关过）——「击败某个难度的 Boss」这类条件读它 |
| `unlockBy.boss(id)` | 判定 `enemyWins[id] >= 1`；`text()` 里的敌人名靠 `setEnemyNames()` **注入**（`unlock.ts` 不能引 `zones`，会成环）。它的 notice 会进 `zoneNotices()` 的中段 —— 第 1 个 Boss 区域走 `chapter(2, 3)`（无 notice），提示手写在 `unlockNotices` 末尾 |

**二、特性（`config/traits.ts` + `playerAttack`）**

- 特性是**装备自己的效果**、对所有怪物生效；`Item.traits?: string[]` 是**物品定义**上的标签、不进存档
- ⚠️ 2026-09-18 起**没有「门槛特性」**：`Enemy.requires` / `ZoneDifficulty.requires / immune` 两个字段、
  「没带就受伤 ×2.5 / 伤害 ÷3」两条倍率、战斗卡片那行 `.requirement`、图鉴怪物页的「特性需求」段**整条移除** ——
  Boss 的难度只由身板与数值决定（原来那套会把肉搏 Boss 变成「翻包找钥匙」）
- 目前**唯一有效果的是【破甲】**：`playerAttack` 里 `armor = hasTrait('piercing') ? 0 : enemy.defense`（无视固定防御）。
  判定仍只有 `hasTrait()` 一处 —— 别写进属性 getter，那会污染卡片与工坊的计算（R34）
- 【隔热】`insulated` = **受到的伤害 −15%**（`enemyAttack` 里按比例减），对所有敌人生效。
  ⚠️ 数值只有一处：`config/traits.ts` 的 `TRAIT_EFFECT.insulatedDamageCut`，文案跟着它生成 —— 别在两处各写一个 15%
- ⚠️ **两条特性各在一件上**（裂甲锥 = 破甲、熔壳护芯 = 隔热）；**遗锋不带任何特性** ——
  它的取向是纯数值 + 可成长（见本节「五」），别顺手给它补特性
- 显示两处缺一不可：装备详情一行「特性：破甲（攻击时无视敌人的固定防御）」、图鉴物品页的「特性」段 ——
  **必须带效果说明**，只给名字等于没给（浮层里只能用 phrasing 元素，见 §7.7）

**三、手动模式与技能（`config/skills.ts` + `pages/adventure.ts`）**

- 解锁：`unlockBy.chapter(2, 4)`（第二章第 4 节「击碎熔核」）；未解锁**整块不渲染**（R29）
- 版式：3 行 4 列 —— 第 1 列是自动 / 手动切换（`grid-row: 1 / -1`），右侧 3×3 是技能槽（行 = 攻击 / 防御 / 辅助）。`.skill-grid` 用 `display: contents`，九个槽位直接参与外层网格
- 槽位内容只有一行：「图标 + 名称」并排（紧凑，别把控制区撑高）；**冷却中整块换成倒计时数字**（`.skill-face` 让位给 `.skill-cd`，两态二选一，不要叠在一起）。倒计时向上取到 0.1 秒，避免最后半帧显示「0.0s」却点不动
- 槽位上**不写技能等级**（`Lv.N` 只在军事训练页 —— 等级是那边练出来的东西，控制区只管这一下打不打得出）
- 槽位下标 = `config/skills.ts` 的下标（**`state.skills` / `adventure.skillTimers` 都按下标存**，顺序不能动；占位槽 `placeholder: true` 不参与任何计算）
- 手动模式 = **玩家侧不自动出手**（`advanceAdventure` 里 `playerWait = Infinity`），敌人照常按自己的间隔出手；玩家出手只由 `useSkill()` 触发
- 【普通攻击】的冷却**跟随出手间隔**（`fromAttackInterval`）—— 手动不会比自动更快，价值在技能与节奏（R35）
- 已实现的技能（其余仍是占位 `???`，`unlock.done = () => false`）：
  - 槽位 0【普通攻击】：按一次打一下，伤害 = `攻击 − 敌防`（【破甲】可无视防御）× `1 + (等级−1) × 1.5%`
  - 槽位 3【格挡】（防御排第 1）：**不开伤害**，只挂 `adventure.blockTimer = BLOCK.window`（3 秒）
  - ⚠️ **格挡是「窗口 + 消耗」而不是持续减伤**：窗口内的**下一记**伤害按 `blockCut(等级)` 减少，
    挨完立刻清零（`enemyAttack` 里结算）；3 秒没挨打就白用一次（`advanceSkillTimers` 每 tick 递减，
    **脱战也走**，所以它不会跨着离线一直留着）
  - 减伤曲线：`20% + 每级 0.1%`，**300 级之后每级只按 1/10 计**，上限 90% ——
    数值只有 `config/skills.ts` 的 `BLOCK` / `blockCut()` 一处，卡片与结算都读它
- 冷却记在 `adventure.skillTimers`（秒），随 tick 递减、脱战也走

**四、军事训练（`pages/research.ts` 的第三个页签）**

- 门槛与手动模式同一个：`unlockBy.chapter(2, 4)`；页签可见性读 `isMilitaryUnlocked()`
- ⚠️ **与工坊制造完全同构**（2026-09-18 改）：每个技能一个**后勤位**（`drillSlot(slot)`，接在制造项之后）、
  进度存 `state.drills[slot] = { target, work }`（形状与 `campWorkshop` 一样）、`advanceDrills()` 挂在 `advanceLogistics()` 末尾。
  派人 + 材料够就自动开工、练完自动接续下一级；人手为 0 就停住（浮层里写「没有人手已暂停」）。
  **可以同时练几项** —— 人手是唯一的闸（原「一次只能训一项 + 待命 ≥ 2 的门槛」已作废）
- ⚠️ 训练**不吃**「勤务」词条加成（那条写的是工坊工时）：基础产出固定 1 工时/人/秒
- 卡片**复用工坊的 `.shop-item`**：图标 / 名称 / 等级在左 + 进度条，右侧是 `summary(level)` 的紧凑数值与「需要 / 现有」材料行，
  派人用同一套 `.shop-assign` 步进器 —— `matMarkup` / `stepperMarkup` / `handleStepperClick` 两页共用 `src/shop-card.ts`，
  **改样式时别只改一页**；说明与状态留在 `.shop-detail` 悬停浮层里。只额外补 `.locked`（虚框）一档状态（**没有「已练满」**，在练的用现成的 `.busy`）
- ⚠️ **未解锁的卡片不显示材料行**：那三行只剩标签、没有数字，看着像坏了 ⇒ `setHidden(材料行, !unlocked)`。
  `.shop-mats` 是 `display: grid`，会把 `[hidden]` 的默认 `display: none` 盖掉，所以额外写了 `.shop-mats[hidden] { display: none; }`
- 「高成本极低增长」：成本 ×1.35/级、**总工时** ×1.15/级、每级效果只 +1.5%（都在 `MILITARY` 与 `SKILL_LEVEL` 里，待调）
- ⚠️ **技能没有等级上限**（2026-09-18）：`SKILL_LEVEL` 只有 `damagePerLevel`，坡度全靠成本与工时撑 ——
  所以界面上**没有「已练满」这一档**（卡片只显示 `Lv.N`，没有分母）
- 占位技能显示「未解锁」+ `???`（不给名字、不给效果），也**不给派人**（步进器整排禁用）
- 存档：**v9**（`state.military` → `state.drills`；不写转换函数，旧记录里那一项训练作废）

**五、可成长饰品与升华（`Item.growth` + `EquipmentInstance.stage`）**

- 阶：`clampStage()` 夹进 `[1, maxStage]`（读档与 `getStage()` 共用）；自身属性 = `基础 ×（1 + (阶−1) × 0.25）×（1 + 精炼/100）`
- ⚠️ **遗锋的基础值是均衡的**：攻 5 / 血 5 / 防 5（1 阶 0 精炼）—— 满配 30/30/30，别按「对齐熔火之核」写
- ⚠️ **成就「Boss 征服者」给饰品槽 +1**：判定与生效都只在 `getEquipSlotCounts()`（现算成就，不另存状态），
  `syncEquipSlots` 会自动补 / 截槽位；页面**每次 update 都要重新读槽位数**（装备页靠 `slotSignature` 决定重建）
- 升华 `ascendEquipment()`：条件是「有 `growth` + 精炼 = 100（【极致】）+ 阶 < maxStage」⇒ 阶 +1、**精炼清零**（不可撤销）
- ⚠️ **它不叫 Boss 的名字**：设定是「一件遗落在这个世界的神兵，能量已散尽」（**遗锋**）——
  名字与描述都不带「熔核 / 守卫」的痕迹，掉落只是它出现的方式
- ⚠️ **不给提示**：描述 / 图鉴 / 帮助 / 更新公告一个字都不提；**界面上也不显示「第几阶」**
  （详情 / 图鉴 / 卡片三处都没有这个字段，成长只从数值上看出来）；「升华」只出现在右键菜单那个按钮上（R36）
- `perfectItems`（曾达到极致的记录）**不受影响**：升华清的是精炼等级，不是「达成过」的记录

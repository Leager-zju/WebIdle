import { items, itemCategories, categoryOrder, devStats, devGrantItem, devSetStat, devUnlockSystems, devCompleteMainline, getState, getOwnedCount, fontScales, getFontScale, setFontScale, numberFormats, getNumberFormat, setNumberFormat, setNotify, formatNumberExact, exportSaveText, importSaveText } from '../game-state';
...
/* ——— 开发者面板 ———
   ...
   devGrantItem / devSetStat / devUnlockSystems / devCompleteMainline / devStats 都会作为死代码从产物里消失，
...
function devItemButtonMarkup(id: number): string { const item = items[id]; return `<button class="dev-item-button ${rarityClass(item.rarity)}" type="button" data-dev-grant="${id}"><span class="dev-item-icon">${item.icon}</span><span class="dev-item-name">${item.name}</span><span class="dev-item-owned" data-dev-owned="${id}"></span></button>`; }
/** 按类别分组（58 件物品平铺在一个网格里，找起来只能一行行扫）。
    类别顺序与名称直接读 config/items.ts —— 和物品栏的页签同源，不在这里另抄一份。
    空类别整块不渲染（类别是配置表驱动的，将来加一类忘了补物品也不会留个空标题）。 */
function devItemSectionsMarkup(): string {
  return categoryOrder.map(category => {
    const ids = items.map((item, id) => (item.category === category ? id : -1)).filter(id => id >= 0);
    if (!ids.length) return '';
    return `<section class="dev-item-section"><h4>${itemCategories[category].name}<span class="dev-item-count">${ids.length} 件</span></h4><div class="dev-item-grid">${ids.map(devItemButtonMarkup).join('')}</div></section>`;
  }).join('');
}
/** 悬浮窗只建一次，之后复用；挂在 body 上，不受设置页重绘影响。 */
function ensureDevModal(): HTMLElement { if (devModalEl) return devModalEl; const modal = document.createElement('div'); modal.className = 'dev-modal-layer'; modal.hidden = true; modal.innerHTML = `<div class="dev-modal dev-item-modal"><div class="dev-modal-head"><div><span class="panel-kicker">DEVELOPER</span><h3>增加物品</h3></div><button class="dev-modal-close" type="button" data-dev-close aria-label="关闭">×</button></div>${devItemSectionsMarkup()}<div class="dev-modal-foot"><span class="dev-multiplier-label">每次发放</span><div class="segmented">${DEV_AMOUNTS.map(amount => `<button class="segment" type="button" data-dev-multiplier="${amount}">×${amount}</button>`).join('')}</div></div></div>`; modal.addEventListener('click', event => { const target = event.target as Element; /* 点关闭按钮、或点在遮罩本身上（不是它的子节点）都关窗。 */ if (target.closest('[data-dev-close]') || target.classList.contains('dev-modal-layer')) { closeDevModal(); return; } const amountButton = target.closest<HTMLElement>('[data-dev-multiplier]'); if (amountButton) { devMultiplier = Number(amountButton.dataset.devMultiplier); syncDevModal(); return; } const grantButton = target.closest<HTMLElement>('[data-dev-grant]'); if (grantButton) { devGrantItem(Number(grantButton.dataset.devGrant), devMultiplier); syncDevModal(); } }); document.addEventListener('keydown', event => { if (event.key === 'Escape') closeDevModal(); }); document.body.appendChild(modal); devModalEl = modal; return modal; }
/** 同步档位高亮与每个物品按钮上的持有数。 */

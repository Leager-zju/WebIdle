...
function statsWindowMarkup(state: GameState): string { const rows = equipBonusStats.map(stat => { const sources = getEquipBonusSources(stat.key, state); const total = sources.reduce((sum, source) => sum + source.value, 0); const suffix = stat.kind === 'pct' ? '%' : ''; const lines = sources.length ? sources.map(source => `<div class="equip-stat-source ${source.colorClass}"><span>${source.name}</span><b>+${formatNumber(source.value)}${suffix}</b></div>`).join('') : '<div class="equip-stat-source empty"><span>还没有装备提供这项加成。</span></div>'; return `<div class="equip-stat ${total ? '' : 'empty'}"><div class="equip-stat-head"><span class="equip-stat-label">${stat.label}</span><b class="equip-stat-value">+${formatNumber(total)}${suffix}</b></div>${lines}</div>`; }).join(''); return `<div class="stats-window"><div class="stats-window-head"><div><span class="panel-kicker">EQUIPMENT BONUS</span><h3>装备加成</h3></div><button class="stats-window-close" type="button" data-stats-close aria-label="关闭">×</button></div><p class="stats-window-copy">只统计已装备物品的贡献：装备自身属性 + 它带的词条。</p><div class="stats-window-list">${rows}</div><div class="stats-window-sets"><span class="panel-kicker">SET BONUS</span>${setRowsMarkup(state)}</div></div>`; }

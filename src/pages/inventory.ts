...
/* 「阶」只在数值里生效，界面上一个字段都不露（原来在详情里跟过一个「第 N 阶」的标签，已撤）。 */
/** 系统物品（`item.system`，钥匙一类）不给「丢弃」—— 目前没有物品用到这个标记。
    （game-state 的 discardItem 与 trimInventoryOverflow 也各挡了一道。） */
/* 「升华」：只有可成长饰品、精炼到【极致】、还没到顶阶时才会多出这一项。
   ⚠️ 这四个字是这套机制**唯一**露面的地方（描述 / 图鉴 / 帮助 / 公告都不提），不要往别处搬。
   位置排在类别操作之后、丢弃之前 —— 它是一个正式操作，不该掉在破坏性操作后面。 */
const ASCEND_ACTION: ItemAction = { id: 'ascend', label: () => '升华（这是什么？）' };
function actionsFor(item: any, instance?: EquipmentInstance | null) {
  const actions = [...(isWikiUnlocked(getState()) ? [WIKI_ACTION] : []), ...(CATEGORY_ACTIONS[item.category] || [])];
  if (canAscend(instance)) actions.push(ASCEND_ACTION);
  if (!item.system) actions.push(...BASE_ACTIONS);
  return actions;
}
/* 装备卡片显示装备自身的属性 + 它带的词条（词条名按品阶着色，数值后面用灰字跟上上限）；
   强化道具显示附加的词条与首次 / 重复强化效果；其他消耗品显示使用效果。 */
function statLine(itemId: number, instance?: EquipmentInstance | null): string { const item = items[itemId]; const lines = []; if (item.equip) { /* 属性走实例：精炼会把它按等级放大，不能直接读配置里的基础值。 */ const bonus = instance ? getInstanceBonus(instance) : { attack: item.equip.attack || 0, hp: item.equip.hp || 0, defense: item.equip.defense || 0 }; const parts = []; if (bonus.attack) parts.push(`攻击 +${bonus.attack}`); if (bonus.hp) parts.push(`生命 +${bonus.hp}`); if (bonus.defense) parts.push(`防御 +${bonus.defense}`); if (parts.length) lines.push(`<span class="item-stats">${parts.join(' · ')}</span>`); } for (const affix of instance?.affixes || []) { const definition = affixes[affix.id]; lines.push(`<span class="item-stats">${affixNameMarkup(affix.id)}：${affixMarkup(affix)}</span>`); } if (item.grantsAffix !== undefined) { const definition = affixes[item.grantsAffix]; lines.push(`<span class="item-stats">附加词条：${affixNameMarkup(item.grantsAffix)} - ${definition.desc}</span>`); /* 一次性强化项（`affixUnique`，例如余烬核心）**不列**「首次 / 重复 / 最高」：它只能刻一条、数值也不再成长，那三行对它没有意义（数值已经写在上面那句 desc 里）。 */ if (!item.affixUnique) { const unit = definition.unit; lines.push(`<span class="item-stats">首次 <b class="affix-value">+${definition.base}${unit}</b></span>`); lines.push(`<span class="item-stats">重复 <b class="affix-value">+${definition.step}${unit}</b></span>`); lines.push(`<span class="item-stats">最高 <b class="affix-value">+${affixCap(item.grantsAffix)}${unit}</b></span>`); } } else if (item.useText) lines.push(`<span class="item-stats">${item.useText}</span>`); /* 套装部件要写清归属与进度：玩家得知道它属于哪套、还差几件。 */
const setId = setOfItem(itemId);
if (setId >= 0) { const entry = setTable[setId]; const worn = getSetWorn(setId); lines.push(`<span class="item-stats">${entry.icon} ${entry.name}套装 · 已穿 ${worn}/${entry.pieces.length}</span>`); } /* 特性（只有带标签的装备才有这一行）：特性是装备自己的效果，**效果描述要跟着写出来**——
   只给「破甲」两个字，玩家看不出它做了什么。 */ if (item.traits?.length) lines.push(`<span class="item-stats item-traits">特性：${item.traits.map(id => `${traitName(id)}（${traitDesc(id)}）`).join(' · ')}</span>`); return lines.join(''); }
...
function detailMarkup(itemId: number, instance: EquipmentInstance | null, equipped: boolean): string {
  ...
  /* ⚠️ 阶**不显示**：详情、图鉴、卡片三处都不写「第几阶」——
     那套（阶与升华）留给玩家自己发现，名字右边只跟精炼那一截（见 庇护所扩展方案 §4.13）。 */
...
function syncUseHint(): void { const hint = ensureHint(); if (usingItemId < 0) { hint.hidden = true; return; } const item = items[usingItemId]; /* 限定强化物（例如余烬核心只吃武器）在提示里就写清可点的目标 —— 点错目标只会留在战斗日志里，玩家在这页看不见。 */ const target = item.affixEquipType !== undefined && equipTypes[item.affixEquipType] ? `点击一件${equipTypes[item.affixEquipType].name}` : '点击一件装备或装备槽'; hint.innerHTML = `<span class="enhance-hint-icon">${item.icon}</span><b>${item.name}</b><span class="muted">${item.useText || '使用'} · ${target} · 其他任意点击取消</span>`; hint.hidden = false; }

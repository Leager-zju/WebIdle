import { items, rarities, affixes, affixMarkup, affixCap, itemCategories, categoryOrder, equipTypes, getEquipSlotCounts, formatNumber, getInventoryCapacity, getInventoryUsed, getInstanceItemId, getEquipmentInstance, getInstanceBonus, equipBonusStats, getEquipBonusSources, isEquipped, getState, discardItem, discardEquipment, useItem, equipItem, equipToSlot, sets, setTable, setOfItem, getSetWorn, setBonusText, getRefine, canRefineWith, refineWithFeeder, REFINE_MAX, isPerfectItem, getPerfectBonus, perfectBonusText } from '../game-state';
...
/** 词条名，颜色跟随品阶。 */
function affixNameMarkup(affixId: number): string { const definition = affixes[affixId]; return `<span class="item-affix-name ${rarityClass(definition.tier)}">${definition.name}</span>`; }
...
function cardMarkup(itemId: number, quantity: number, instance: EquipmentInstance | null): string { const item = items[itemId]; const equipped = !!instance && isEquipped(instance.id); const draggable = item.category === 'equipment' ? ' draggable="true"' : ''; const refine = getRefine(instance || undefined); return `<article class="item-card ${rarityClass(item.rarity)} ${equipped ? 'equipped' : ''}" data-item="${itemId}" data-instance="${instance ? instance.id : -1}"${draggable} tabindex="0"><div class="item-icon">${item.icon}${instance && refine ? `<span class="item-refine${refine >= REFINE_MAX ? ' perfect' : ''}">+${refine}</span>` : ''}</div>${item.stackable ? `<strong class="item-quantity">×${formatNumber(quantity)}</strong>` : ''}<div class="item-detail"><b class="item-detail-name ${rarityClass(item.rarity)}">${item.name}${instance ? refineMarkup(refine) : ''}</b><span class="item-type">${item.type} · ${rarities[item.rarity].name}${equipped ? ' · 已装备' : ''}</span><p>${item.description}</p>${statLine(itemId, instance)}</div></article>`; }
/* 卡片来源：inventory 是可堆叠物品的数量，equipment 是装备实例（一件一张卡片）。
...
function cardsOf(state: GameState, tab: string): string[] { const cards: string[] = []; state.inventory.forEach((quantity, itemId) => { if (quantity > 0 && inTab(items[itemId].category, tab)) cards.push(cardMarkup(itemId, quantity, null)); }); [...state.equipment].sort((a, b) => a.itemId - b.itemId).forEach(instance => { if (inTab(items[instance.itemId].category, tab)) cards.push(cardMarkup(instance.itemId, 1, instance)); }); return cards; }
...
function statsWindowMarkup(state: GameState): string { const rows = equipBonusStats.map(stat => { const sources = getEquipBonusSources(stat.key, state); const total = sources.reduce((sum, source) => sum + source.value, 0); const suffix = stat.kind === 'pct' ? '%' : ''; const lines = sources.length ? sources.map(source => `<div class="equip-stat-source ${rarityClass(source.tier)}"><span>${source.name}</span><b>+${formatNumber(source.value)}${suffix}</b></div>`).join('') : '<div class="equip-stat-source empty"><span>还没有装备提供这项加成。</span></div>'; return `<div class="equip-stat ${total ? '' : 'empty'}"><div class="equip-stat-head"><span class="equip-stat-label">${stat.label}</span><b class="equip-stat-value">+${formatNumber(total)}${suffix}</b></div>${lines}</div>`; }).join(''); return `<div class="stats-window"><div class="stats-window-head"><div><span class="panel-kicker">EQUIPMENT BONUS</span><h3>装备加成</h3></div><button class="stats-window-close" type="button" data-stats-close aria-label="关闭">×</button></div><p class="stats-window-copy">只统计已装备物品的贡献：装备自身属性 + 它带的词条。来源名字的颜色代表品阶。</p><div class="stats-window-list">${rows}</div><div class="stats-window-sets"><span class="panel-kicker">SET BONUS</span>${setRowsMarkup(state)}</div></div>`; }
...
function setRowsMarkup(state: GameState): string {
  ...
  return rows.map(({ entry, setId, worn }) => {
    ...
    return `<div class="equip-stat ${active ? '' : 'empty'}"><div class="equip-stat-head"><span class="equip-stat-label">${entry.icon} ${entry.name}套装</span><b class="equip-stat-value">${worn} / ${total}</b></div><div class="set-bonus">${active ? '已激活：' : '凑齐后：'}${setBonusText(entry.bonus)}</div><div class="set-pieces">${pieces}</div></div>`;
  }).join('');
}
...
/* 结构签名只关心「有哪些卡片」：装备实例、可堆叠物品的种类（有/无）、词条与当前页签。
   数量不进签名，否则每次掉落都会重绘整片网格 —— 正在悬停的卡片会被换掉，浮层的位置与内容一起跳。 */
const signature = `${state.equipped.map(slots => slots.join('.')).join(',')}|${state.inventory.map(quantity => (quantity > 0 ? 1 : 0)).join(',')}|${state.equipment.map(instance => `${instance.id}:${(instance.affixes || []).map(affix => `${affix.id}.${affix.value}`).join('+')}`).join(',')}|${ctx.category}`;

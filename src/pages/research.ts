import {
  ...
  isResearchItemUnlocked, getResearchReward, items, zones, mainline, formatNumber
} from '../game-state';
...
const page: PageDefinition<any> = {
  ...
  mount(root) {
    ...
    const ctx: any = {
      ...pick(view, 'points', 'desc', 'requirement', 'submit'),
    ...
    root.onclick = event => {
      ...
      if (target.closest<HTMLElement>('[data-action="submit"]')) { submitResearchTask(); return; }
  ...
  update(state: GameState, ctx: any) {
    ...
    setText(ctx.points, `研究点数 ${formatNumber(state.researchPoints)}`);
    const zone = zones[task.zoneId];
    /* 一句话说完：要什么、去哪儿，物品名与区域名加粗，其余交给下面的 requirement。 */
    setHtml(ctx.desc, item && zone
      ? `基地正在分析异常电池，需要一批<b>${item.icon} ${item.name}</b>。到<b>${zone.name}</b>狩猎，把掉落物带回来即可交付。`
      : '暂时没有可发布的委托：先在冒险里解锁新的区域。');
    /* 条件行沿用远征档案的 .requirement：物品名写成「▣ 旧电池」并加粗（同 chapter-fact 的值），够了就点亮。 */
    ctx.requirement.innerHTML = item ? `<div class="requirement ${done ? 'done' : ''}"><span class="status-dot ${done ? '' : 'pending'}"></span><span><b>${item.icon} ${item.name}</b> ${formatNumber(owned)}/${formatNumber(task.need)}</span></div>` : '';

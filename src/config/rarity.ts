/* 稀有度表：对象字面量的键顺序就是 rarity id（0,1,2…），新增稀有度在末尾追加即可。
   ...
   目前已用到 0 / 1 / 2 / 14 / 15 / 16 六档 —— 战斗区域 + 三档「特殊渠道」：

     灰色（0）  废弃边境
     白色（1）  余烬矿脉 / 核心深井
     蓝色（2）  熔火裂谷 / Boss 独占（熔核之扉）
     火红色（14）特殊渠道物品（清洗剂、地图碎片 —— 不走 dropTable 的那两条通道）
     琥珀色（15）任务物品（跨区域，所以单独占最高一档，不和区域抢位置）
     靛蓝色（16）系统物品（钥匙一类 —— 排在任务物品之后，和区域、和上面两条渠道都不抢位置）

   ⚠️ **两套装备可以同色**：稀有度只是色彩档、不是进度刻度，颜色重叠不代表强度重叠
   （2026-09-18 调整：核心套降一档、熔火套降一档，Boss 独占用蓝 2）。
   绿色（3）起是留给 Boss 之后的新区域 / 新世界的空位，颜色已经在 style.css 里备好。

...
const ENEMY_DEFS = {
  ...
  wirehound: { art: '犬', name: '线缆猎犬', description: '用断裂电缆当腿的四足机械，扑咬极快，但一撞就散。', maxHp: 64, attack: 6, defense: 0, attackInterval: 1.7, gold: 15, dropTable: [{ itemId: ITEM.scrap, chance: 1, min: 2, max: 4 }, { itemId: ITEM.whetOil, chance: .08, min: 1, max: 1 }, { itemId: ITEM.oldBattery, chance: .2, min: 1, max: 1 }] },
  scrapGolem: { art: '偶', name: '废铁傀儡', description: '被人一层层焊上废铁的搬运单位，外壳厚得离谱，攻击却没什么章法。', maxHp: 130, attack: 7, defense: 2, attackInterval: 2.5, gold: 30, dropTable: [{ itemId: ITEM.scrap, chance: 1, min: 6, max: 10 }, { itemId: ITEM.armorPlate, chance: .34, min: 1, max: 1 }, { itemId: ITEM.platingGoo, chance: .06, min: 1, max: 1 }] },
  ...
  ashCrawler: { art: '爬', name: '灰烬爬行者', description: '在矿道顶壁上爬行的多足机械，落下来时带着一身高热灰。', maxHp: 300, attack: 18, defense: 6, attackInterval: 1.5, gold: 65, dropTable: [{ itemId: ITEM.scrap, chance: 1, min: 5, max: 9 }, { itemId: ITEM.emberShard, chance: .34, min: 1, max: 1 }, { itemId: ITEM.platingGoo, chance: .07, min: 1, max: 1 }] },
  veinWarden: { art: '卫', name: '矿脉守卫', description: '守着主矿脉的重甲单位，装甲板就是从它身上撬下来的标准件。', maxHp: 380, attack: 14, defense: 9, attackInterval: 1.1, gold: 105, dropTable: [{ itemId: ITEM.armorPlate, chance: .6, min: 1, max: 2 }, { itemId: ITEM.scrap, chance: 1, min: 8, max: 12 }, { itemId: ITEM.emberCore, chance: .06, min: 1, max: 1 }] },
  emberLeech: { art: '蛭', name: '余烬水蛭', description: '吸附在结晶上取热的软体机械，被打散时会溅出灼热的体液。', maxHp: 280, attack: 26, defense: 6, attackInterval: 2.4, gold: 100, dropTable: [{ itemId: ITEM.emberShard, chance: .4, min: 1, max: 1 }, { itemId: ITEM.lifeSeed, chance: .08, min: 1, max: 1 }, { itemId: ITEM.scrap, chance: 1, min: 4, max: 8 }] },
  moltenHound: { art: '渣', name: '熔渣猎犬', description: '关节处凝着熔渣的追猎单位，越打越烫。', maxHp: 300, attack: 20, defense: 8, attackInterval: 1.8, gold: 120, dropTable: [{ itemId: ITEM.scrap, chance: 1, min: 6, max: 10 }, { itemId: ITEM.emberCore, chance: .07, min: 1, max: 1 }] },

  /* ——— 核心深井（enemyId 10-14）：信号最深处，井壁上全是结晶 ———
     玩家此时大约「攻 50 / 防 8 / 血 260 / 回复 6 / 间隔 2.1」（余烬套装齐 + 营火 5 级），
     净损失压在 83~85；泰坦作为终点故意留高一档（112，约 43% 生命）。 */
  coreDrone: { art: '机', name: '核心无人机', description: '围绕井口盘旋的维护机，炮口对准一切还在动的东西。', maxHp: 400, attack: 22, defense: 12, attackInterval: 1.4, gold: 175, dropTable: [{ itemId: ITEM.scrap, chance: 1, min: 10, max: 16 }, { itemId: ITEM.emberCore, chance: .08, min: 1, max: 1 }, { itemId: ITEM.logisticsManual, chance: .08, min: 1, max: 1 }] },
  ...
  echoWraith: { art: '影', name: '回声残影', description: '一段没能散掉的旧信号，出手快得只留下一道残影。', maxHp: 380, attack: 38, defense: 10, attackInterval: 2.8, gold: 225, dropTable: [{ itemId: ITEM.emberCore, chance: .09, min: 1, max: 1 }, { itemId: ITEM.emberShard, chance: .4, min: 1, max: 2 }] },
  abyssBrute: { art: '渊', name: '深渊重装者', description: '井底的重载单位，每一块装甲都比远征队整支队伍还厚。', maxHp: 620, attack: 24, defense: 18, attackInterval: 2, gold: 255, dropTable: [{ itemId: ITEM.armorPlate, chance: 1, min: 2, max: 3 }, { itemId: ITEM.scrap, chance: 1, min: 12, max: 18 }, { itemId: ITEM.emberCore, chance: .1, min: 1, max: 1 }] },
  coreTitan: { art: '枢', name: '核心泰坦', description: '井底那颗仍在搏动的核心本身。所有信号的源头。', maxHp: 640, attack: 36, defense: 18, attackInterval: 3.2, gold: 365, dropTable: [{ itemId: ITEM.emberCore, chance: .2, min: 1, max: 1 }, { itemId: ITEM.armorPlate, chance: 1, min: 3, max: 4 }, { itemId: ITEM.emberShard, chance: .8, min: 2, max: 3 }] }
...
/* 区域表：键顺序就是 zoneId。enemyIds 引用敌人表下标；
   ...
   只索取任务物品，一个区域一种，该区域**所有**怪物统一 QUEST_DROP_CHANCE 掉落。
...
const ZONE_DEFS = {
  ...
  emberVein: { name: '余烬矿脉', icon: '💠', description: '被结晶污染的旧矿井，渗出的热量让整条矿道都在发光。守卫这里的单位已经开始结晶化。', enemyIds: [ENEMY.emberMite, ENEMY.ashCrawler, ENEMY.veinWarden, ENEMY.emberLeech, ENEMY.moltenHound], unlockIndex: 5, dropRefine: 5, questItem: ITEM.crystalSample },

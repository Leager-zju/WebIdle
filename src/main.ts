...
function updateSharedHeader(state: ReturnType<typeof getState>): void { status.classList.toggle('paused', !state.adventure.running); setHtml(statusText, state.adventure.running ? `远征中 · ${zoneRefMarkup(currentZoneId(state))}` : '庇护所待命'); updateNavLocks(state); navItems.forEach(item => item.classList.toggle('active', item.dataset.page === pageController.currentId)); }
